...
export async function generatePlanilha21kmPdf(opts: {
  ...
  function newPage() {
    ...
    const title = "PLANILHA 21KM";
    const titleW = bold.widthOfTextAtSize(title, 16);
    drawText(page, title, A4.w - margin - titleW, A4.h - 30, bold, 16, white);
    const subRaw = `${PHASE_LABELS_21KM[currentPhase].title} — ${PHASE_LABELS_21KM[currentPhase].subtitle}`;
  ...
  weeks.forEach((wk, wi) => {
    ...
    wk.assignments.forEach((a) => {
      ...
      const intense = WORKOUT_TYPES_21KM[wo.type].intense;
