...
export const getPlanilha21kmData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: plans } = await supabaseAdmin
      ...
      .eq("student_id", data.studentId).eq("plan_type", "21km").eq("status", "ativa")
...
export const savePlanilha21kmConfig = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: existing } = await supabaseAdmin
      .from("training_plans")
      .select("id").eq("student_id", data.studentId).eq("plan_type", "21km").eq("status", "ativa")
      .order("updated_at", { ascending: false }).limit(1);
