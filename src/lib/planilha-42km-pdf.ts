...
const REMINDERS = [
  "Fazer os exercícios de Alongamento, Mobilidade e Fortalecimento;",
  "Após o aquecimento fazer os educativos de corrida;",
  "Passar o feedback dos treinos, nem que seja apenas um \"tudo certo\";",
  "Sincronizar o treino no seu aplicativo de GPS e no Strava;",
...
export async function generatePlanilha42kmPdf(opts: {
  ...
  function newPage() {
    ...
    const title = "PLANILHA 42KM";
    const titleW = bold.widthOfTextAtSize(title, 16);
    drawText(page, title, A4.w - margin - titleW, A4.h - 30, bold, 16, white);
    const subRaw = `${PHASE_LABELS_42KM[currentPhase].title} — ${PHASE_LABELS_42KM[currentPhase].subtitle}`;
  ...
  const infoLine = `Gerado em ${today}  •  Nível ${level}  •  ${daysPerWeek} dias: ${weekDays.map((d) => DAY_LABEL[d]).join(", ")}`;
  ...
  weeks.forEach((wk, wi) => {
    ...
    wk.assignments.forEach((a) => {
      ...
      const intense = WORKOUT_TYPES_42KM[wo.type].intense;
  ...
  page.drawRectangle({ x: margin, y: y - 22, width: contentW, height: 22, color: secondary });
  drawText(page, "LEMBRAR SEMPRE!", margin + 8, y - 16, bold, 11, white);
  y -= 30;
  REMINDERS.forEach((r) => {
    for (const ln of wrap(`• ${r}`, font, 10, contentW - 14)) {
      ensure(13);
      drawText(page, ln, margin + 8, y, font, 10, ink);
      y -= 13;
    }
    y -= 2;
  });
