// Cálculo dinâmico de duração e volume da Planilha 42km a partir do FTP do aluno.
import type { Item, ZoneId } from "./planilha-5km-data";
import type { Workout42km, Phase42 } from "./planilha-42km-data";
import { WORKOUTS_42KM } from "./planilha-42km-data";
...
export function computeWorkoutStat(wo: Workout42km, ftp: number): { durationMin: number; volumeM: number } {
  let sec = 0, m = 0;
...
export function makeStatsLookup42km(ftpSecPerKm: number) {
  return (level: 1 | 2, phase: 1 | 2 | 3 | 4, weekIdx: number, code: string) => {
    const wos = WORKOUTS_42KM[level]?.[phase as Phase42]?.[weekIdx];
    if (!wos) return null;
