...
export const getPlanilha42kmData = createServerFn({ method: "GET" })
...
export const savePlanilha42kmConfig = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: existing } = await supabaseAdmin
      .from("training_plans")
      .select("id, payload, start_date").eq("student_id", data.studentId).eq("plan_type", "42km").eq("status", "ativa")
      .order("updated_at", { ascending: false }).limit(1);
