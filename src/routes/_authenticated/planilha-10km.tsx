...
function Planilha10kmPage() {
  const qc = useQueryClient();
  const getDataFn = useServerFn(getPlanilha10kmData);
  const saveFn = useServerFn(savePlanilha10kmConfig);
  ...
  const dataQuery = useQuery({
    enabled: !!studentId,
    queryKey: ["planilha-10km", studentId],
  ...
  async function handleExportPdf() {
    ...
    setExporting(true);
    try {
      const blob = await generatePlanilha10kmPdf({
      ...
      downloadBlob(blob, `Planilha-10km-${safeName}-Fase${phase}.pdf`);
    ...
    <div className="max-w-6xl space-y-6">
      <div>
        <nav className="flex items-center gap-1 text-sm text-muted-foreground mb-2">
          ...
          <span className="text-foreground">Planilha 10KM</span>
        </nav>
        <h1 className="text-3xl font-display font-bold">Planilha 10KM</h1>
        <p className="text-muted-foreground">Monte a planilha do aluno com paces personalizados a partir do Teste 3km.</p>
      </div>

      <DistanceSelector current="10km" />
          ...
          <CardHeader className="flex-row items-center justify-between space-y-0">
            ...
            <div className="flex gap-2">
              ...
              <PlanStartDatePicker
                ...
                invalidateQueryKey={["planilha-10km", studentId] as const}
