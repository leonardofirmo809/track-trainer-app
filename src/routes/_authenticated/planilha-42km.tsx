...
import { generatePlanilha42kmPdf, downloadBlob } from "@/lib/planilha-42km-pdf";
import { GarminExportDialog, type GarminWeek, type GarminSession } from "@/components/planilha/GarminExportDialog";
import { makeStatsLookup42km, formatKm, formatKm2, formatHms } from "@/lib/planilha-42km-stats";
...
import {
  WORKOUTS_42KM, WORKOUT_TYPES_42KM, PHASE_LABELS_42KM, DAY_ORDER, DAY_LABEL, DAY_FULL,
  type DayCode, type Workout42km as Workout, type Item, type SectionName, type ZoneId, type Phase42,
} from "@/lib/planilha-42km-data";
import { distributeWeek, type DistributionResult } from "@/lib/planilha-5km-distribute";
import { validateWeekDays42km } from "@/lib/planilha-42km-distribute";
import { getPlanilha42kmData, savePlanilha42kmConfig } from "@/lib/planilha-42km.functions";
...
export const Route = createFileRoute("/_authenticated/planilha-42km")({ component: Planilha42kmPage });

const PHASES: Phase42[] = [1, 2, 3, 4, 5];
...
function Planilha42kmPage() {
  const qc = useQueryClient();
  const getDataFn = useServerFn(getPlanilha42kmData);
  const saveFn = useServerFn(savePlanilha42kmConfig);

  const [studentId, setStudentId] = useState<string>("");
  const [level, setLevel] = useState<1 | 2>(1);
  const [weekDays, setWeekDays] = useState<DayCode[]>([]);
  const [phase, setPhase] = useState<Phase42>(1);
  ...
  const dataQuery = useQuery({
    enabled: !!studentId,
    queryKey: ["planilha-42km", studentId],
  ...
  const statsLookup = useMemo(() => makeStatsLookup42km(ftpSec), [ftpSec]);
  ...
  const validation = useMemo<string | null>(() => validateWeekDays42km(level, weekDays), [level, weekDays]);
  ...
  const allPhasesBlocks = useMemo((): PhaseBlock[] | null => {
    ...
    return ([1, 2, 3, 4, 5] as const).map((p) => {
      ...
      const totals = computePhaseTotals(
        WORKOUTS_42KM[level][p] as never,
        level, phaseAs4, statsLookup,
      );
      return {
        phaseNum: p,
        label: PHASE_LABELS_42KM[p].title,
        subtitle: PHASE_LABELS_42KM[p].subtitle,
  ...
  const weeks = useMemo(() => {
    ...
    return phaseWeeks.map((wos, w) => {
      ...
      return distributeWeek(list, weekDays, level, WORKOUT_TYPES_42KM, { manualDayByCode, noDrop: true });
    });
  ...
  function changePhase(p: Phase42) {
    setPhase(p);
  ...
  async function handleExportPdf() {
    ...
    setExporting(true);
    try {
      const blob = await generatePlanilha42kmPdf({
    ...
    <div className="max-w-6xl space-y-6">
      <div>
        <nav className="flex items-center gap-1 text-sm text-muted-foreground mb-2">
          ...
          <span className="text-foreground">Planilha 42KM</span>
        </nav>
        <h1 className="text-3xl font-display font-bold">Planilha 42KM — Maratona</h1>
        <p className="text-muted-foreground">Monte a planilha de maratona com paces personalizados a partir do FTP do aluno.</p>
      </div>

      <DistanceSelector current="42km" />
        ...
        <Card>
          <CardHeader className="flex-row items-start justify-between space-y-0 flex-wrap gap-3">
            <CardTitle className="pt-0.5 flex items-center gap-2 flex-wrap">
              4. Planilha e treinos <PlanSourceBadge source={activeSource} />
            </CardTitle>
          ...
          <CardContent className="space-y-4">
            ...
            <Tabs value={String(phase)} onValueChange={(v) => changePhase(Number(v) as Phase42)}>
              <TabsList>
                {PHASES.map((p) => (
              ...
              {PHASES.map((p) => (
                <TabsContent key={p} value={String(p)} className="space-y-6">
                  <div>
                    <p className="text-sm text-muted-foreground">{PHASE_LABELS_42KM[p].subtitle}</p>
                  </div>
                  {weeks.map((wk, idx) => (
...
function WeekRow({ index, dist, level, phase, weekIdx, statsLookup, onOpen }: {
  index: number; dist: DistributionResult<Workout>; level: 1 | 2; phase: Phase42; weekIdx: number;
            ...
            <div key={a.day}>
              {a.workout ? (
                <button
                  ...
                  className={`w-full text-left rounded-md border-2 p-3 hover:opacity-90 transition ${WORKOUT_TYPES_42KM[a.workout.type].color}`}
                >
...
function PhaseChartsBlock({ weeksWorkouts, level, phase, statsLookup }: {
  weeksWorkouts: Workout[][]; level: 1 | 2; phase: Phase42; statsLookup: StatsLookup;
}) {
  const phaseAs4 = phase as 1 | 2 | 3 | 4;
